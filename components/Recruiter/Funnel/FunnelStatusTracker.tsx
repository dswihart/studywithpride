  ...
  if (loading) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <div className="flex items-center justify-center py-8" role="status" aria-label="Loading funnel status">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
          <span className="sr-only">Loading funnel status...</span>
        </div>
      </div>
    )
  }
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <div className="text-center py-6">
          ...
          {lead.student_id && (
            <a
              href="/admin/students"
              className="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors focus:ring-2 focus:ring-green-400 focus:outline-none"
            >
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
      <div className="p-5 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white" id="funnel-heading">Recruitment Funnel</h2>
          <span className="text-sm text-gray-500 dark:text-gray-400" aria-live="polite">{completedCount} of 4 complete</span>
        </div>
      </div>

      <div className="p-5">
        {/* Accessible progressbar wrapper */}
        <div
          role="progressbar"
          ...
          aria-valuemin={0}
          aria-valuemax={4}
          aria-valuetext={completedCount + " of 4 stages complete"}
        >
              ...
              return (
                <div key={stage.key} className="flex flex-col items-center flex-1">
                  <div className="flex items-center w-full">
                    {index > 0 && (
                      <div className={"h-0.5 flex-1 " + (prevComplete ? "bg-green-500" : "bg-gray-300 dark:bg-gray-600")} aria-hidden="true" />
                    )}
                    <div
                      className={"w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 " + (complete ? "bg-green-500" : "bg-gray-300 dark:bg-gray-600")}
                      aria-label={stage.label + (complete ? " - Complete" : " - Incomplete")}
                    >
                      {complete ? (
                        ...
                        <span className="text-white text-sm font-bold" aria-hidden="true">{stage.number}</span>
                      )}
                    </div>
                    {index < FUNNEL_STAGES.length - 1 && (
                      <div className={"h-0.5 flex-1 " + (complete ? "bg-green-500" : "bg-gray-300 dark:bg-gray-600")} aria-hidden="true" />
                    )}
                  </div>
        ...
        <div aria-live="assertive" aria-atomic="true">
          {error && (
            <div className="mt-4 p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800" role="alert">
        ...
        {allComplete && !lead.converted_to_student && onConvertToStudent && (
          <div className="mt-6">
            <button
              ...
              disabled={converting}
              className="w-full py-3 px-4 bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white font-semibold rounded-lg transition-colors flex items-center justify-center gap-2 focus:ring-2 focus:ring-green-400 focus:outline-none"
            ...
            >
              {converting ? (
                ...
                <>
                  ...
                  <span>Convert to Student Portal</span>
                </>
              )}
            </button>
          </div>
