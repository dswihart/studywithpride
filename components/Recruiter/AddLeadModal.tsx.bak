      ...
      if (duplicateLeads.length > 0) {
        for (const row of duplicateLeads) {
          ...
          if (existingLead) {
            ...
            if (!existingLead.name_score && row.data.name_score) {
              updates.name_score = parseInt(row.data.name_score)
            }
            if (!existingLead.email_score && row.data.email_score) {
              updates.email_score = parseInt(row.data.email_score)
            }
            if (!existingLead.lead_score && row.data.lead_score) {
              updates.lead_score = parseInt(row.data.lead_score)
            }
            if (!existingLead.lead_quality && row.data.lead_quality) {
              updates.lead_quality = row.data.lead_quality
            }
            if (!existingLead.recency_score && row.data.recency_score) {
              updates.recency_score = parseInt(row.data.recency_score)
            }
